# Documentation

## Integrate the editor

- [Getting started](./getting-started.md)
- [Controlled React integration](./controlled-react.md)
- [React integration lifecycle](./integration-lifecycle.md)
- [Email integration](./email-integration.md)
- [Headless core](./headless-core.md)
- [Custom profiles](./custom-profiles.md)
- [Rich-text editing](./rich-text.md)
- [Slide decks](./decks.md)
- [HTML and deck imports](./importing.md)
- [Element behavior](./element-behavior.md)
- [Security model](./security.md)
- [Runtime and framework compatibility](./compatibility.md)
- [HTML compatibility matrix](./html-compatibility.md)
- [Versioning and API stability](./versioning.md)
- [Public declaration reference](../api/README.md)

## Contribute and maintain

- [Development setup](../CONTRIBUTING.md)
- [Contributor map](./contributor-map.md)
- [Implemented architecture](../ARCHITECTURE.md)
- [Test architecture](../tests/README.md)
- [Scenario matrix](../TEST_CASES.md)
- [Architecture decisions](./adr/README.md)
- [Roadmap](../ROADMAP.md)
- [Release procedure](../RELEASING.md)
- [Public release checklist](./public-release-checklist.md)
- [Governance](../GOVERNANCE.md)

## Context and historical records

- [Historical design proposals](./design/README.md)
- [Correctness review and follow-up](./reviews/2026-09-11-correctness-review.md)
- [Asset provenance and visual identity](./visual-design.md)
- [Landing demo implementation](./editing-demo.md)

Current guides describe implemented contracts and executable checks. Historical
documents retain the assumptions and results of their date; consult the current
architecture and roadmap before treating a proposal as supported functionality.
