...
test('font family selector remains legible and previewable', async ({ page }) => {
  await page.goto('/#/editor');
  await page.frameLocator('iframe[title="Visual HTML canvas"]').locator('h1').click();
  await page.locator('[data-style-property="font-family"] [role="combobox"]').click();
  const menu = page.getByRole('listbox', { name: 'Font options' });
  await expect(menu).toBeVisible();
  await stabilizeVisuals(page);
  await expect(menu.locator('..')).toHaveScreenshot('font-family-selector.png', {
    animations: 'disabled', caret: 'hide', maxDiffPixelRatio: 0.02
  });
});
