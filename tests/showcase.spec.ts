...
test('applies inspector style changes as source-backed patches', async ({ page }) => {
  ...
  await page.locator('[data-style-property="font-family"] [role="combobox"]').click();
  await expect(page.getByRole('listbox', { name: 'Font options' })).toBeVisible();
  const arialOption = page.getByRole('option', { name: 'Arial Web-safe sans serif' });
  await expect(arialOption).toHaveCSS('cursor', 'pointer');
  await arialOption.click();
  await expect(frame.locator('h1')).toHaveCSS('font-family', 'Arial, Helvetica, sans-serif');
  ...
  await expect(page.getByLabel('HTML source')).toContainText('font-family: Arial');
...
test('keeps layer labels and tag badges inside the navigator', async ({ page }) => {
  const heading = page.getByRole('treeitem', { name: /Heading: “Build skills that move/ });
  const select = heading.getByRole('button', { name: /Select Heading:/ });
  const tag = heading.locator('.vhe-outline-node__tag');
  await expect(select).toHaveAttribute('title', /Heading: “Build skills that move with you.” \(h1\)/);
  await expect(tag).toHaveText('<h1>');
  await expect(tag).toHaveAttribute('title', '<h1>');

  const navigatorBox = await page.locator('.vhe-navigator').boundingBox();
  const tagBox = await tag.boundingBox();
  expect(navigatorBox).not.toBeNull();
  expect(tagBox).not.toBeNull();
  expect(tagBox!.x + tagBox!.width).toBeLessThanOrEqual(navigatorBox!.x + navigatorBox!.width);
});
...
test('manages and exports a multi-slide HTML deck', async ({ page }) => {
  ...
  await expect(slideOptions.first()).toHaveCSS('cursor', 'pointer');
  ...
  const duplicateSlide = page.getByRole('menuitem', { name: /Duplicate slide/ });
  await expect(duplicateSlide).toHaveCSS('cursor', 'pointer');
  await duplicateSlide.click();
...
test('styles a selected text range with typography, emphasis, and block alignment', async ({ page }) => {
  ...
  await expect(page.getByLabel('HTML source')).toContainText('font-family: Arial, Helvetica, sans-serif');
