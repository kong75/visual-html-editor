...
function compactOutlineText(value: string, limit = 30): string {
...
export function ComponentOutlineNode({ node, nodes, selectedKey, collapsedKeys, depth, behaviorResolvers, onSelect, onToggle }: ComponentOutlineNodeProps) {
  ...
  return (
    ...
    >
      <div className="vhe-outline-node__row" style={{ paddingLeft: 6 + depth * 14 }}>
        {children.length ? (
        ...
        <button type="button" className={`vhe-outline-node__select${name ? '' : ' vhe-outline-node__select--unnamed'}`} onClick={() => onSelect(node.key)} aria-label={`Select ${accessibleName}`}>
