...
import { EditorNavigator } from './workspace/navigator.js';
  ...
  return (
    <div className={`vhe-root ${className}`} data-testid="visual-html-editor" data-read-only={readOnly || undefined}>
      <EditorToolbar brandHref={brandHref} documentTitle={documentTitle} toolbarContent={toolbarContent}
        ...
        mode={mode} canUndo={!readOnly && (snapshot.canUndo || textEditing.textHistoryState.canUndo)}
        canRedo={!readOnly && (snapshot.canRedo || textEditing.textHistoryState.canRedo)} openSource={openSource} setMode={setMode}
        runHistory={textEditing.runtime.runHistory} preserveActiveTextEdit={(event) => { if (textEditing.runtime.activeRichTextEditRef.current) event.preventDefault(); }}
      ...
      <div className={`vhe-layout${navigationRail ? ' vhe-layout--with-navigation' : ''}`}>
        <EditorNavigator navigationRail={navigationRail} selectedKey={selectedKey} outline={outline} elementBehaviorResolvers={elementBehaviorResolvers} />
        ...
        <EditorSidebar snapshot={editorSnapshot} selectedNode={selectedNode}
          outline={outline} inspector={inspector}
          elementBehaviorResolvers={elementBehaviorResolvers} sidebarHeader={sidebarHeader}
          onImage={() => imageInputRef.current?.click()} readOnly={readOnly} />
      </div>
