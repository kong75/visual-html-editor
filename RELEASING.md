...
Publish only from a provenance-capable runner checking out that tag. The runner must use the committed lockfile, Node 24.11.1, pnpm 10.22.0, and npm 11.5.1 or newer. Use GitHub Actions with `contents: read` and `id-token: write`, and configure the matching npm trusted publisher before using OIDC. Initial package registration or alternate publishing credentials remain owner setup work.
